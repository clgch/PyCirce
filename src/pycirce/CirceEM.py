...
class CirceEM:
    ...
    def __init__(
        ...
        if h is None:
            raise ValueError(
                "Please provide a Jacobian matrix h of size (n_parameters, n_observations) of the thermohydraulic numerical simulator"
            )
        if z_nom is None:
        ...
        if not (h.shape[1] == len(z_exp) == len(z_nom)):
            raise ValueError(
                "ERROR: size inconsistencies between the Jacobian h, the vector z_exp and z_nom !"
            )
    ...
    def _one_step_em(mean, cov, h, z_exp, z_nom, sig_eps, n):
        ...
        for i in range(n):
            ...
            b.append(
                cov
                @ (h_i * ((z_exp[i] - z_nom[i]) - h_i.T @ mean))
        ...
        cov_new = (
            copy.deepcopy(cov) + np.mean(delta_cov, axis=0) - delta_mean @ delta_mean.T
        )
    ...
    def estimate(self):
        ...
        mean_new, cov_new = self._one_step_em(
            self.initial_mean,
            self.initial_cov,
            self.h,
            self.z_exp,
        ...
        rel_diff = np.abs(cov_list[-1] - cov_list[-2]) / np.abs(cov_list[-1])
        ...
        while (
            err_cov > self.tolerance or err_mean > self.tolerance
        ) and iterator < self.niter:
            loglik_list += [
                self._loglik(
                    mean_list[-1],
                    cov_list[-1],
