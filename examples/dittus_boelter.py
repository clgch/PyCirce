...
def dittus_boelter_corr(x):
    ...
    return np.log10(0.023) + np.log10(lambda_f/ D) + np.log10(G * D / mu_f) * 0.8 + np.log10(c_p * mu_f / lambda_f) * 0.4
...
def generate_data_becker(data, seed):
    ...
    gamma_true = np.diag(np.array([0.1, .1, .1]))
    ...
    return np.log10(0.023) + np.log10(lambda_f/ D) * theta[:, 0] + np.log10(G * D / mu_f) * theta[:, 1] + np.log10(c_p * mu_f / lambda_f) * theta[:, 2]
...
circe_diag = pyc.CirceEMdiag(initial_mean=[1, 0.8, 0.4], initial_cov=np.identity(3), h=h, z_exp=z_sim, z_nom=np.repeat(np.log10(0.023), n), sig_eps=sig_eps, niter=1000)
