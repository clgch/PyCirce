...
def dittus_boelter_corr(x):
    ...
    return (
        0.023 * (lambda_f / D) * (G * D / mu_f) ** 0.8 * (c_p * mu_f / lambda_f) ** 0.4
    )
...
def generate_data_becker(data, std, seed):
    ...
    Tp = data[:, 2] + np.random.uniform(
        -std[:, 2] * np.sqrt(3), std[:, 2] * np.sqrt(3), size=n
    )
    D = data[:, 3] + np.random.uniform(
        -std[:, 3] * np.sqrt(3), std[:, 3] * np.sqrt(3), size=n
    )
    ...
    return (
        0.023
        * np.power((lambda_f / D), theta[:, 0])
        * np.power((G * D / mu_f), theta[:, 1])
        * np.power((c_p * mu_f / lambda_f), theta[:, 2])
    )
